...
assert_contains(index, ['Hero', '7-Day Desk Worker Recomp Reset', 'Fitsek 12-Week Recomp System', 'Privacy', 'Terms', 'Fitness disclaimer', 'leads.fitsek.com/signup', 'static.cloudflareinsights.com/beacon.min.js'], 'index.html')
for section in ['problem','how','preview','faq','signup']:
    assert f'id="{section}"' in index, f'missing section #{section}'
for claim in ['diagnose','disease','hormone cure','guaranteed fat loss','before/after']:
...
for required in ['CNAME','robots.txt','sitemap.xml','AGENTS.md','AI_STATE.md','.gitignore']:
    assert (ROOT/required).exists(), f'missing {required}'
# Secret hygiene: key names may appear in docs/examples, but values should not look like live secrets.
